...
import { executeMarketingComposition } from "@ceo-agent/agents/photo-scene/execute-marketing-composition";
import {
...
function asGenerationSnapshot(
  ...
  return {
    ...
    inputCapsule: PhotoSceneMarketingInputCapsuleV1Schema.parse(row.inputCapsule),
...
export async function processPhotoSceneComposeJob(data: {
  generationId: string;
  workspaceId: string;
  orgId: string;
  campaignId: string;
  ...
  if (!generation) {
    emitPhotoSceneOpsEvent({
      ...
      failureClass: "INVALID_EXTRACTED_PRODUCT",
    });
    return;
  }
  ...
  const workDir = join(tmpdir(), `photo-scene-compose-${generation.id}`);
  ...
  try {
    const result = await executeMarketingComposition({
      ...
      io: {
        ...
        readExtractedBytes: async (storagePath) => {
          const localPath = join(workDir, "extracted.png");
          await downloadStorageFile(storagePath, localPath);
          return readFile(localPath);
        },
        readSceneBytes: async (identity) => {
          const localPath = join(workDir, "scene.png");
          await downloadStorageFile(identity.objectKey, localPath, { bucket: identity.bucket });
          return readFile(localPath);
        },
        readLogoBytes: async (identity) => {
          const localPath = join(workDir, "logo.bin");
          await downloadStorageFile(identity.objectKey, localPath, { bucket: identity.bucket });
          return readFile(localPath);
        },
        ...
        loadExtractedAsset: async (assetId) => {
    ...
    emitPhotoSceneOpsEvent({
      event: result.status === "ready" ? "composition.completed" : "composition.failed",
      stage: "photo_scene.compose",
    ...
    const code = isMarketingErrorCategory((err as { code?: string }).code)
      ? (err as { code: "COMPOSITION_FAILED" }).code
      : "COMPOSITION_FAILED";
    await db
      .update(schema.photoSceneGenerations)
      .set({
...
export async function markPhotoSceneComposeJobFailed(data: {
  generationId?: string;
  workspaceId?: string;
  orgId?: string;
  campaignId?: string;
  ...
  await db
    ...
    .set({
      ...
      boundedError: userSafeMarketingMessage("COMPOSITION_FAILED"),
      completedAt: new Date(),
      updatedAt: new Date(),
    })
    .where(
