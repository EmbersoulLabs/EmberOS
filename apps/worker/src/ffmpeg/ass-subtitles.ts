...
function assStyleLine(
  ...
  bold = -1,
  shadow = 2,
  outlineColor: string = ASS_COLOR_BLACK
): string {
  const fontName = subtitleFontName();
  return (
    `Style: ${name},${fontName},${fontSize},${ASS_COLOR_WHITE},${ASS_COLOR_BLACK},${outlineColor},&H80000000,` +
...
export function buildAssSubtitles(
  ...
  options?: { brandColor?: string | null; bounce?: boolean }
): string {
  ...
  const brandColor = options?.brandColor?.trim() || null;
  const outlineColor = brandColor ?? ASS_COLOR_BLACK;
  // Brand color outline needs a visible border even on "minimal" (outline 0).
  const baseOutline = brandColor ? Math.max(cfg.outlinePx, 2) : cfg.outlinePx;
  const outline = Math.max(0, Math.round(baseOutline * scale));
...
${assStyleLine("BrandHook", Math.round(56 * scale), 1, Math.round(120 * scale), outline, 56, -1, shadow, outlineColor)}
${assStyleLine("BrandBody", primarySize, 2, marginV, outline, 48, cfg.primaryBold ? -1 : 0, shadow, outlineColor)}
${assStyleLine("BrandBodySecondary", secondarySize, 2, marginV - Math.round(36 * scale), outline, 48, cfg.secondaryBold ? -1 : 0, shadow, outlineColor)}
Style: Default,${font},${primarySize},${ASS_COLOR_WHITE},${ASS_COLOR_BLACK},${outlineColor},&H80000000,-1,0,0,0,100,100,0,0,1,${outline},${shadow},2,48,48,${marginV},1
...
`;
  ...
  const lines = subtitles.map((s: EditPlan["subtitles"][number]) => {
    ...
    const text = buildAssAnimatedDialogueText(s.text, highlightKeywords, cfg, {
      ...
      pop: !bounce,
      bounce,
      durationMs: Math.max(0, Math.round((s.endSec - s.startSec) * 1000)),
    });
