...
import { episodeMomentMarker, type GeneratedSceneReviewReadModel, type HumanCreativeRejectionReason, type WorkspaceRole } from "@ceo-agent/shared";
import {
...
function statusLabel(state: GeneratedSceneReviewReadModel["runtimeState"]): string {
  return ({ AUTHORIZED_NOT_RELEASED: "Ready — waiting for the previous moment", QUEUED: "Ready", PRE_DISPATCH_BLOCKED: "Generation couldn’t start", RUNNING: "Generating", PENDING_REVIEW: "Needs review", REJECTED: "Needs changes", RETRY_AUTHORIZED: "Needs changes", APPROVED: "Approved", FAILED: "This moment could not be generated." } as const)[state];
}
...
function repairAction(owner: string): string {
  if (owner === "PROVIDER_EXECUTION") return "Try generating this moment again";
  ...
  if (owner === "SCRIPT" || owner === "SCENE") return "Edit what happens in this moment";
  if (owner === "DIRECTOR" || owner === "MOTION") return "Adjust the moment action";
...
export function SceneReviewWorkspacePanel({ campaignId, storyId, executionPlanId, workspaceRole, scenes, onChanged }: Props) {
  ...
  if (!scenes.length) return <section className="rounded-2xl border border-dashed border-border bg-white p-6 text-center" data-testid="scene-review-empty"><h2 className="text-lg font-bold text-navy">Episode moments</h2><p className="mt-2 text-sm text-ink-secondary">No moments are ready for generation or review yet.</p></section>;
  ...
  return <section className="space-y-4" data-testid="scene-review-workspace" aria-labelledby="scene-workspace-heading">
    <div className="rounded-2xl border border-border bg-white p-5 sm:flex sm:items-end sm:justify-between sm:gap-4"><div><h2 id="scene-workspace-heading" className="text-xl font-bold text-navy">Episode moments</h2><p className="mt-1 text-sm text-ink-secondary">See who and what matters, then review each generated result.</p></div><p className="mt-3 text-sm font-semibold text-navy sm:mt-0" data-testid="story-review-progress">{approved} of {scenes.length} moments ready</p></div>
    <nav className="flex gap-2 overflow-x-auto pb-1" aria-label="Episode moment navigation" data-testid="scene-navigation">{scenes.map((scene) => <a key={scene.sceneExecutionId} href={`#scene-${scene.sceneOrder + 1}`} className="shrink-0 rounded-full border border-border bg-white px-3 py-1.5 text-sm text-navy hover:bg-surface-muted">{episodeMomentMarker(scene.sceneOrder)}</a>)}</nav>
    <div className="space-y-4">{scenes.map((scene, index) => {
      ...
      return <article id={`scene-${scene.sceneOrder + 1}`} key={scene.sceneExecutionId} className="scroll-mt-4 overflow-hidden rounded-2xl border border-border bg-white" data-testid={`scene-review-card-${scene.sceneOrder}`}>
        ...
        <footer className="flex items-center justify-between border-t border-border px-4 py-3 text-sm sm:px-5"><span className="text-ink-secondary">{episodeMomentMarker(scene.sceneOrder)}</span><div className="flex gap-3">{index > 0 ? <a href={`#scene-${scene.sceneOrder}`} className="text-brand-blue hover:underline">Previous</a> : null}{index < scenes.length - 1 ? <a href={`#scene-${scene.sceneOrder + 2}`} className="text-brand-blue hover:underline">Next moment</a> : null}</div></footer>
      </article>;
    })}</div>
