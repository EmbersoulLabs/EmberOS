const expected = process.argv[2]?.trim() || process.env.EXPECTED_EMBEROS_RELEASE_REVISION?.trim();
const origin = process.argv[3]?.trim() || process.env.EMBEROS_PRODUCTION_ORIGIN?.trim();
