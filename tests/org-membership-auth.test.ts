...
function memberRow(
  overrides: Partial<OrganizationMemberRow> & Pick<OrganizationMemberRow, "orgId" | "userId" | "role">
...
describe("requireOrganizationMembership (real authorization path)", () => {
  ...
  it("does not grant access from membership in a different organization", async () => {
    const lookup = vi.fn(async (requestedOrgId: string) => {
      // User is only a member of Org A — never return a row for Org B.
  ...
  it("TenantValidationError is distinct from OrganizationAccessError", () => {
    ...
    const forbidden = new OrganizationAccessError("Not a member of this organization", "FORBIDDEN");
