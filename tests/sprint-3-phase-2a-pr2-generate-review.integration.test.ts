...
describeIntegration("Sprint 3 Phase 2A PR2 Generate Review persistence", () => {
  ...
  beforeAll(async () => {
    ...
    await sql`UPDATE ai_stories SET current_version_id = ${PHASE_2A_IDS.storyVersionId} WHERE id = ${PHASE_2A_IDS.storyId}`;
    await sql`INSERT INTO ai_story_animation_packages (id, org_id, workspace_id, campaign_id, story_id, story_version_id, status, payload, approved_at, approved_by) VALUES (${PHASE_2A_IDS.animationPackageId}, ${PHASE_2A_IDS.orgId}, ${PHASE_2A_IDS.workspaceId}, ${PHASE_2A_IDS.campaignId}, ${PHASE_2A_IDS.storyId}, ${PHASE_2A_IDS.storyVersionId}, 'ready_for_execution', ${sql.json(payload)}, NOW(), ${PHASE_2A_IDS.orgId})`;
