...
describeIntegration("AI Story canonical Scene service lifecycle against aggregate v2",()=>{
  ...
  afterAll(async()=>{await closeDb();if(!sql)return;await sql`delete from ai_story_canonical_scene_versions where story_id=${I.story}::uuid`;await sql`delete from ai_story_canonical_scenes where story_id=${I.story}::uuid`;await sql`delete from ai_story_script_versions where story_id=${I.story}::uuid`;await sql`delete from ai_story_outline_versions where story_id=${I.story}::uuid`;await sql`delete from ai_story_character_versions where character_id=${I.character}::uuid`;await sql`delete from ai_story_characters where character_id=${I.character}::uuid`;await sql`delete from ai_story_versions where story_id=${I.story}::uuid`;await sql`delete from ai_stories where id=${I.story}::uuid`;await cleanupRlsFixture(sql,fixture);await sql.end();},30_000);
