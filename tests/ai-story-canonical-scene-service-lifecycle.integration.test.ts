...
describeIntegration("AI Story canonical Scene service lifecycle against aggregate v2",()=>{
  ...
  it("persists exact canonical lineage, rejects stale revisions, and excludes legacy authority",async()=>{
    ...
    expect(currentV1![0]!.generationAuthority).toEqual(REFERENCE_FREE);
    const durableModeV1=await sql`select snapshot->'generationAuthority' as mode from ai_story_canonical_scene_versions where scene_version_id=${firstScene.sceneVersionId}::uuid`;
    expect(durableModeV1[0]!.mode).toEqual(REFERENCE_FREE);
    const packageFixture=animationPackageFixture("ready_for_execution");
    packageFixture.scenePlan[0]!.generationAuthority=REFERENCE_FREE;
    ...
    expect(persisted.intents[0]!.generationAuthority).toMatchObject({strategy:"TEXT_TO_VIDEO",referenceSource:"REFERENCE_FREE_T2V",effectiveReferenceIds:[],firstFrameAssetId:null});
    expect(persisted.instructionsBySceneExecutionId[persisted.intents[0]!.identity.sceneExecutionId]?.generationAuthority).toEqual(persisted.intents[0]!.generationAuthority);
