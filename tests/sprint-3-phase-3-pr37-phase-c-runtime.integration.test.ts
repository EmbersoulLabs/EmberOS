/**
 * Sprint 3 PR 3.7 Phase C remediation â€” real PostgreSQL full-chain E2E.
...
describeIntegration("Sprint 3 PR 3.7 Phase C remediation Postgres E2E", () => {
  ...
  it("B: multi-scene success â†’ one Assembly Job / Artifact / FSR", async () => {
  ...
  it("C: terminal failure â†’ DEAD_LETTER, no usage/cost, no Assembly/FSR", async () => {
    await cleanupPr32Tenant(sql);
  ...
  it("D: ACCEPTANCE_UNKNOWN â†’ reconciliation â†’ terminal success (no resubmit)", async () => {
    await cleanupPr32Tenant(sql);
    ...
    // Same tick may advance PROCESSING â†’ SUCCEEDED via bound lookup (no resubmit).
  ...
  it("E: transient infrastructure â†’ no terminal Finalizer / no Scene Result", async () => {
    await cleanupPr32Tenant(sql);
  ...
  it("F: duplicate Dispatch delivery â†’ one terminal accepted effect", async () => {
    await cleanupPr32Tenant(sql);
  ...
  it("G: restart/resume after accepted submit â†’ lookup, zero resubmit", async () => {
    await cleanupPr32Tenant(sql);
  ...
  it("H: 10-way continuation concurrency â†’ one Assembly Job / Artifact / FSR", async () => {
    if (!RUN_FFMPEG) return;
    await cleanupPr32Tenant(sql);
