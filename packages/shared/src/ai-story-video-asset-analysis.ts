...
const AiStoryVideoAssetObservationObject = z.object({
  ...
  observedActions: z.array(z.string().trim().min(1).max(80)).max(12).default([]),
  observedObjects: z.array(z.string().trim().min(1).max(80)).max(12).default([]),
  ...
  observedEnvironments: z.array(z.string().trim().min(1).max(80)).max(12).default([]),
