...
import { type AiStoryCanonicalScene, type AiStoryLocationAuthorityVersion } from "./ai-story-scene";
...
export function evaluateAiStoryPreGenerationQc(raw:AiStoryPreGenerationQcInput):AiStoryPreGenerationQcEvaluation {
  ...
  const results:AiStoryPreGenerationQcGateResult[]=[
    ...
    hard("SCRIPT_TRUTH_PRESERVATION_GATE",[...has(directorIssues,["SCENE_IDENTITY_GATE","CANONICAL_SCENE_BINDING_GATE","SCRIPT_TRUTH_BINDING_GATE","SCRIPT_ACTION_SUPPORT_GATE"],"DIRECTOR","DIRECTOR"),...has(motionIssues,["CANONICAL_SCENE_BINDING_GATE","SCRIPT_ACTION_TRUTH_GATE"],"MOTION","MOTION")],ids),
...
export function assertAiStoryPreGenerationQcCurrent(evaluation:AiStoryPreGenerationQcEvaluation,current:{outlineVersionId:string;scriptVersionId:string;handoffId:string;directorPlanId:string;motionPlanId:string;sceneVersionIds?:readonly string[]}){if(evaluation.outlineVersionId!==current.outlineVersionId||evaluation.scriptVersionId!==current.scriptVersionId||evaluation.handoffId!==current.handoffId||evaluation.directorPlanId!==current.directorPlanId||evaluation.motionPlanId!==current.motionPlanId)throw new Error("STALE_PREGEN_QC_DENIED");if(evaluation.sceneVersionIds&&JSON.stringify([...evaluation.sceneVersionIds].sort())!==JSON.stringify([...(current.sceneVersionIds??[])].sort()))throw new Error("STALE_PREGEN_QC_SCENE_DENIED");}
