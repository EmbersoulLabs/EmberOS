...
export function computeAiStoryPreGenerationQcFingerprint(input:Pick<AiStoryPreGenerationQcEvaluation,"orgId"|"workspaceId"|"storyId"|"storyVersionId"|"outlineVersionId"|"scriptVersionId"|"handoffId"|"directorPlanId"|"motionPlanId"|"sceneExecutionId"|"gateSetVersion"|"providerCapabilityId"|"providerCapabilityVersion"|"productAuthorityIds"|"gateResults"|"recipeGateResults"|"shotRecipeBindings"|"dispatchDecision">){
  ...
  return sha256CanonicalIntegrityHash({contractVersion:AI_STORY_PRE_GENERATION_QC_CONTRACT_VERSION,orgId:input.orgId,workspaceId:input.workspaceId,storyId:input.storyId,storyVersionId:input.storyVersionId,outlineVersionId:input.outlineVersionId,scriptVersionId:input.scriptVersionId,handoffId:input.handoffId,directorPlanId:input.directorPlanId,motionPlanId:input.motionPlanId,sceneExecutionId:input.sceneExecutionId,gateSetVersion:input.gateSetVersion,providerCapabilityId:input.providerCapabilityId,providerCapabilityVersion:input.providerCapabilityVersion,productAuthorityIds:input.productAuthorityIds,gateResults:input.gateResults,...recipeEvidence,dispatchDecision:input.dispatchDecision});
}
...
export function evaluateAiStoryPreGenerationQc(raw:AiStoryPreGenerationQcInput):AiStoryPreGenerationQcEvaluation {
  ...
  const results:AiStoryPreGenerationQcGateResult[]=[
    ...
    hard("PRODUCT_AUTHORITY_CAUSALITY_CONTINUITY_GATE",[...productReasons,...profileIssues.filter(i=>i.severity==="BLOCK"&&["PRODUCT_AUTHORITY_GATE","CLAIM_EVIDENCE_GATE"].includes(i.gate)).map(i=>reason(i.reasonCode,i.message,"PRODUCT_AUTHORITY","PRODUCT_AUTHORITY"))],ids),
