...
try {
  for (const fileName of ["ai-story-staged-release-v1.sql", "ai-story-staged-release-rls-v1.sql"]) {
    const statements = readFileSync(resolve(directory, `../sql/${fileName}`), "utf8").split(";").map((s) => s.trim()).filter(Boolean);
    for (const statement of statements) await client.unsafe(statement);
  }
} finally { await client.end(); }
console.log("AI Story staged release schema applied.");
