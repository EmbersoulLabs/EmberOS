/** Apply VS-RC-01A.3B generation identity. Refuses production. */
...
const sql = readFileSync(
  resolve(directory, "../sql/campaign-video-generation-identity-v1.sql"),
  "utf8"
);
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(sql);
  console.log("Campaign video generation identity schema applied.");
} finally {
  await db.end();
}
