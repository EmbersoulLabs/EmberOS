/** Apply the nullable Story Outline Profile authority column. Refuses Production/R4. */
...
const migration = readFileSync(
  resolve(directory, "../sql/ai-story-outline-profile-authority-v1.sql"),
  "utf8"
);
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(migration);
  console.log("AI Story Outline Profile authority column applied.");
} finally {
  await db.end();
}
