/** Apply Photo Scene 10C official scene catalog. Refuses production. */
...
const sql = readFileSync(resolve(directory, "../sql/photo-scene-official-scenes-v1.sql"), "utf8");
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(sql);
  console.log("Photo Scene official scene catalog schema applied.");
} finally {
  await db.end();
}
