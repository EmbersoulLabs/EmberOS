/** Apply Photo Scene 10A campaign_asset_refs. Refuses production. */
...
const sql = readFileSync(resolve(directory, "../sql/photo-scene-campaign-asset-refs-v1.sql"), "utf8");
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(sql);
  console.log("Photo Scene campaign_asset_refs schema applied.");
} finally {
  await db.end();
}
