/** Apply the canonical approved Animation Package uniqueness invariant. Refuses Production/R4. */
...
const migration = readFileSync(
  resolve(directory, "../sql/ai-story-current-approved-animation-package-authority-v1.sql"),
  "utf8"
);
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(migration);
  console.log("Canonical approved Animation Package authority applied.");
} finally {
  await db.end();
}
