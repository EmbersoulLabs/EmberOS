/** Apply Photo Scene 10B photo_scene_generations. Refuses production. */
...
const sql = readFileSync(resolve(directory, "../sql/photo-scene-generations-v1.sql"), "utf8");
const db = postgres(url, { max: 1 });
try {
  await db.unsafe(sql);
  console.log("Photo Scene photo_scene_generations schema applied.");
} finally {
  await db.end();
}
