/**
 * Apply PD-044 Campaign Description removal (data preserve + DROP COLUMN).
 * Usage: pnpm --filter @ceo-agent/db sql:campaign-description-pd044
 */
...
const sqlFile = resolve(__dirname, "../sql/campaign-description-pd044.sql");
...
try {
  ...
  console.log("PD-044 Campaign Description removal applied (idempotent).");
} finally {
  await db.end();
}
