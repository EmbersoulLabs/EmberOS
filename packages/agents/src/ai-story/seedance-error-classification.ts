/**
 * Sprint 3 PR 3.4A — Seedance error → canonical Worker failure classification.
...
import { SeedanceConfigError } from "./seedance-config";
import { SeedanceHttpTransportError } from "./seedance-http-client";
import { SeedanceMappingError } from "./seedance-request-mapping";

export type SeedanceErrorClass =
...
export type SeedanceErrorPolicy = {
  readonly class: SeedanceErrorClass;
...
const POLICIES: Record<SeedanceErrorClass, SeedanceErrorPolicy> = {
  INFRASTRUCTURE_TRANSIENT: {
...
export function seedanceErrorPolicy(errorClass: SeedanceErrorClass): SeedanceErrorPolicy {
  return POLICIES[errorClass];
}

export function classifySeedanceError(
  error: unknown,
  phase: "submit" | "lookup" | "callback"
): {
  readonly policy: SeedanceErrorPolicy;
  readonly failure: WorkerFailureClassification;
} {
  if (error instanceof SeedanceMappingError) {
  ...
  if (error instanceof SeedanceConfigError) {
  ...
  if (error instanceof SeedanceHttpTransportError) {
  ...
  if (/moderation|safety|content.?policy/i.test(message) || status === 422) {
  ...
  if (status === 400 || status === 403 || /not.?accepted|invalid.?request/i.test(message)) {
  ...
  if (status === 429 || status >= 500 || /unavailable|network|econnreset/i.test(message)) {
    const policy = POLICIES.INFRASTRUCTURE_TRANSIENT;
    return {
      policy,
      failure: failureFromCode(policy.workerCode, policy.safeUserMessage, {
...
export function assertFallbackDisabled(policy: SeedanceErrorPolicy): void {
  if (policy.fallbackAllowed !== false) {
    throw new Error("Seedance Adapter must never allow cross-provider fallback");
  }
}
